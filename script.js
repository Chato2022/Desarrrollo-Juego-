...
function iniciar(event){
    imagen.src = 'imagenes/imagen_0.png';
    btn.disabled = true;
    cant_errores = 0;
    cant_aciertos = 0; 
    frase_resultado="";
    id('resultado').innerHTML =frase_resultado;
...
function click_letras(event){
    ...
    const palabra = palabrita.toUpperCase(); // convertir a mayuscula
    ...
    if( acerto == false ){
        cant_errores++;
        const source = `imagenes/imagen_${cant_errores}.png` ;
        imagen.src = source;
    }

    if( cant_errores == 6 ){
        frase_resultado="Fin del Juego, la palabra era ";
        id('resultado').innerHTML =frase_resultado + palabrita;
        game_over( );
    }else if( cant_aciertos == palabrita.length ){
        frase_resultado="¡Ganaste, Felicidades!"
        id('resultado').innerHTML = frase_resultado;
        game_over( );
    }
